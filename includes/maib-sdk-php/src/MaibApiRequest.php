...
class MaibApi
{
    ...
    public function pay($data, $token)
    {
        ...
        {
            error_log('Invalid request: ' . esc_html($e->getMessage()));
            throw new PaymentException('Invalid request: ' . esc_html($e->getMessage()));
        }
    }
    ...
    private function sendRequestPost($endpoint, $data, $token)
    {
        ...
        {
            throw new PaymentException("HTTP error while sending POST request to endpoint " . esc_html($endpoint) . ": " . esc_html($e->getMessage()));
        }
    ...
    private function sendRequestGet($endpoint, $id, $token)
    {
        ...
        {
            throw new PaymentException("HTTP error while sending GET request to endpoint " . esc_html($endpoint) . ": " . esc_html($e->getMessage()));
        }
        return $this->handleResponse($response, $endpoint);
    }
    ...
    private function sendRequestDelete($endpoint, $id, $token)
    {
        ...
        {
            throw new PaymentException("HTTP error while sending DELETE request to endpoint " . esc_html($endpoint) . ": " . esc_html($e->getMessage()));
        }
        return $this->handleResponse($response, $endpoint);
    }
    ...
    private function handleResponse($response, $endpoint)
    {
        if (isset($response->ok) && $response->ok)
        {
            ...
            {
                throw new PaymentException("Invalid response received from server for endpoint " . esc_html($endpoint) . ": missing 'result' field");
            }
        }
        else
        {
            if (isset($response->errors))
            {
                ...
                throw new PaymentException("Error sending request to endpoint " . esc_html($endpoint) . ": " . esc_html("{$error->errorMessage} ({$error->errorCode})"));
            }
            else
            {
                throw new PaymentException("Invalid response received from server for endpoint " . esc_html($endpoint) . ": missing 'ok' and 'errors' fields");
            }
        }
    }
    ...
    private function validatePayParams($data, $requiredParams)
    {
        ...
        foreach ($requiredParams as $param)
        {
            if (!isset($data[$param]))
            {
                throw new PaymentException("Missing required parameter: " . esc_html("'$param'"));
            }
        }
