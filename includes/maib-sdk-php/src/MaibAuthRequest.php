...
class MaibAuth
{
    ...
    public function generateToken($ProjectIdOrRefresh = null, $projectSecret = null)
    {
        ...
        {
            throw new TokenException(esc_html("Project ID and Project Secret or Refresh Token must be provided!"));
        }
        ...
        if ($ProjectIdOrRefresh !== null && $projectSecret !== null)
        {
            ...
            {
                throw new TokenException(esc_html("Project ID and Project Secret must be strings!"));
            }
        ...
        elseif ($ProjectIdOrRefresh !== null && $projectSecret === null)
        {
            ...
            {
                throw new TokenException(esc_html("Refresh Token must be a string!"));
            }
        ...
        {
            throw new TokenException(esc_html("HTTP error while sending POST request to endpoint generate-token: {$e->getMessage() }"));
        }
    ...
    private function handleResponse($response, $endpoint)
    {
        if (isset($response->ok) && $response->ok)
        {
            ...
            {
                throw new TokenException(esc_html("Invalid response received from server for endpoint $endpoint: missing 'result' field"));
            }
        }
        else
        {
            if (isset($response->errors))
            {
                ...
                throw new TokenException(esc_html("Error sending request to endpoint $endpoint: {$error->errorMessage} ({$error->errorCode})"));
            }
            else
            {
                throw new TokenException(esc_html("Invalid response received from server for endpoint $endpoint: missing 'ok' and 'errors' fields"));
            }
        }
    }
}